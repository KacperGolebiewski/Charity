...
public class PasswordRecoveryService {
    ...
    public String sendToken(String email) {
        ...
        if (!emailExist) {
            throw new IllegalStateException("user with such email does not exist");
        }

        String token = UUID.randomUUID().toString();
        AppUser appUser = appUserRepository.findByEmail(email).get();

        ForgottenPasswordToken forgottenPasswordToken = new ForgottenPasswordToken(token, LocalDateTime.now(), LocalDateTime.now().plusMinutes(15), appUser);

        forgottenPasswordTokenService.saveForgottenPasswordToken(forgottenPasswordToken);
    ...
    public String confirmToken(String token) {
        ...
        LocalDateTime expiredAt = forgottenPasswordToken.getExpiresAt();
